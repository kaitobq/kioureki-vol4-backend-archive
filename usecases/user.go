...
func (u *UserUsecase) CreateUser(name, email, password string) (*response.AuthedUserResponse, error) {
	// メールが既に使用されていないか確認
	...
	return response.NewAuthedUserResponse(*user, token), nil
}

func (u *UserUsecase) Authenticate(email, password string) (*response.AuthedUserResponse, error) {
...
func (u *UserUsecase) GetUserJoinedOrganization(userID uint) (*response.UserJoinedOrganizationResponse, error) {
	...
	return response.NewUserJoinedOrganizationResponse(user, organizations), nil
}
